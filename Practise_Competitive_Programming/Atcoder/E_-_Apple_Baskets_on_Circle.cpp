...
// }
int t;
int checkAns(vector<int>&a, int m) {
    // find number of apples eaten after m laps
    int cnt = 0;
    for(auto i : a) {
        if(i <= m) cnt += i;
        else cnt += m;
...
int binary_search_index(vector<int> &a,int k) {
    int n = a.size(); 
    int l = 0 ; 
    int r = k + 1; 
    // l and r are number of laps
    while( l <= r ) {
        int mid= r +(l-r ) / 2;
        int ans = checkAns(a,mid); 
        if(ans > k) {
            // more apples eaten 
            r = mid-1;
        }
        else if(ans == k) {
            // correct amount of apples eaten 
            cout << "Here1" << endl;
            return ans;
        }
        else {
            // less number of apples eaten
            l = mid +1;
        }
    }
    // l and r both can be the answers I feel.
    // but I think l will be answer, because I started with out of range value for r
    int ansValue = checkAns(a,l);
    if(ansValue > k) {
        cout << "Here2" << endl;
        return ansValue - (ansValue - k);
    }
        return ansValue;
}
signed main(){
