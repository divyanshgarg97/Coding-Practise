...
signed main(){
    ...
    vector<int> dp(n+1,0);
    dp[0] = 0;
    // dp[i] = maximum number of stones player can get when we have 'i' stones.
    for(int i = 1 ;i <= n ;i++) {
        for(int j = 0 ;j < k ;j++) {
            if(v[j] > i) // we cannot draw more than i coins,
                continue;
            dp[i] = max(dp[i], i - dp[i - v[j]]); // If I take v[j] step, then how many coins will I get in total,, ie. i - dp[ i - v[j]]
